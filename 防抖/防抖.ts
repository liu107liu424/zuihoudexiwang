...
let timer: null | number //定义全局 time
input.addEventListener("input", (msg: any) => {
    // 进来之前先判断有没有 计时器  如果有就删除
    // 应为在连续调用的时候 计时器两秒后执行 ， 但是上次的还没执行 这边就有执行了一次事件
...
// 定义全局time 防抖的代码跟逻辑代码 混在一起不容易维护  
// 利用闭包 单独封装起来写一个真正的防抖函数
interface AntiShake {
    (fn: Function, number: number): any
}
let AntiShake: AntiShake = function (fn, time) {
    let timenum: null | number = null
    return function () {
...
/**
 * @处理滑动调用防抖时间 停止多少秒之后执行
 */
interface scrollAntiShake {
    (fn: Function, number: number): any
}
let scrollhua = document.getElementById("scroll")
let scrollAntiShake: scrollAntiShake = function (fn, number) {
    let timer: null | number = null
    return function () {
        let that = this
        // 判断如果timer 说明没到时间还在调用  把当前的timer给清除掉
        if (timer) clearTimeout(timer); //这时候的timer 是numer 掉了多少次的数量
        let dowo: boolean = !timer  //会得到一个布尔值  !null == true  timer不变成null  永远不会执行我们的逻辑代码  取反一个数字就是false
 ...
 */
// 利用闭包原理
let every_other: scrollAntiShake = function (fn, number) {
